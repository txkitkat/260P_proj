...
class Problem3:
    ...
    def isBipartite(self, graph: list, n: int) -> bool:
        ...
        try:
            ...
            for node in range(n):
                if node == 0:
                    if not self.search(graph, node, True):
                        return False
            return True
        except IndexError: 
...
if __name__ == "__main__":
    ...
    x = test_class.isBipartite(test_1, len(test_1))
    print(x)
    y = test_class.isBipartite(test_2, len(test_2))
    print(y)